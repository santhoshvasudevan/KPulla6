...
| `note` | Optional text |
| `is_reversal` | `BooleanField` — reversal row (CASH-CORR-1A) |
| `reverses_id` | Nullable self-FK — original entry reversed |
| `reversal_reason` | Audit text |
| `created_at` / `updated_at` | Timestamps |

Migration: `cash/migrations/0003_cashledgerentry_reversal_fields.py`
