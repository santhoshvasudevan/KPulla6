...
def fixed_deposit_portfolio_mismatch_warning(fd: FixedDeposit) -> str | None:
    """Read-only warning when FD portfolio differs from bank account cash visibility link."""
    ...
    return (
        ...
        f"bank account cash visibility portfolio ({bank_name}). "
        "This does not block FD creation; reconcile only if cash visibility is wrong."
    )
